...
void Solve()
{
    FOR(i, 1, n)
    {
        maximize(dp[i][0], dp[i-1][0]);
        maximize(dp[i][0], dp[i-1][1]);
        maximize(dp[i][0], dp[i-1][2]);

        if(getBit(a[i], 0))
        {
            maximize(dp[i][1], dp[i-1][0]+1);
            maximize(dp[i][1], dp[i-1][2]+1);
        }
        if(getBit(a[i], 1))
        {
            maximize(dp[i][2], dp[i-1][0]+1);
            maximize(dp[i][2], dp[i-1][1]+1);
        }
    }

    cout << n - max(max(dp[n][1], dp[n][2]), dp[n][0]);
}
