...
void Attempt()
{
    int q = 0, cnt = 0;
    crd1 = 0, crd2 = 0;
    for(char c: s) crd1 += (c == '+') ? 1 : -1;
    for(char c: t) if(c == '?') ++q;
    else crd2 += (c == '+') ? 1 : -1;


    if(q) FORl(state, 0, (1 << q))
    {
        int nwcrd = crd2;
        FORl(i, 0, q) if(getBit(state, i)) ++nwcrd; else --nwcrd;
        cnt += int(nwcrd == crd1);
    }
    else cnt = int(crd1 == crd2);

    cout << fixed << setprecision(12) << (long double)(cnt) / (long double)(1 << q) << el;
}
