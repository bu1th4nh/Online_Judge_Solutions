...
//Check
ii extended_euclid(ll a, ll b)
{
    if(!b) return ii(1, 0);
    else
    {
        ii p = extended_euclid(b, a % b);
        return ii(p.second, p.first - (a / b) * p.second);
    }
}
vi congruenceEquation(vi a, int b, int m)
{
    vi ret;
    vi g(n+1), x(n+1);
    a.push_back(m);
    ...
    FORb(i, n, 1)
    {
        ii p = extended_euclid(g[i-1], a[i]);
        x[i] = p.second * val % m;
        val  = p.first * val % m;
    }

    x[0] = val;
    FORl(i, 0, n) x[i] = (x[i] + m) % m;
    FORl(i, 0, n) ret.push_back(x[i]);
...
void Solve()
{
    vi res = congruenceEquation(a, b, m);
    if(res.empty())
    {
        puts("NO");
    }
