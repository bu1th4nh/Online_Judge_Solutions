...
typedef priority_queue<pack> pq;
typedef pair<int, int> ii;
typedef vector<bool> vb;
typedef vector<int> vi;
typedef vector<si> vsi;
typedef vector<ii> vii;
typedef vector<pq> vpq;
typedef vector<vi> vvi;
...
int retriev(int u)
{
    ...
    if(res[u].empty()) return inf;
    while(!res[u].empty())
    {
        i   = res[u].top().id;
        ret = res[u].top().val;
        if(!state[i]) res[u].pop();
        else return ret;
    }
...
void update(int u)
{
    ...
    while(1)
    {
        ...
        res[x].push(pack(u, lca.getDist(x, u)));
        if(x == par[x]) break;
        x = par[x];
    }
}
int query(int u)
{
    ...
    while(1)
    {
        ...
        ret = min(ret, retriev(x) + lca.getDist(x, u));
        if(x == par[x]) break;
        x = par[x];
    }
...
void Solve()
{
    ...
    //FOR(i, 1, n) cerr << par[i] << sp; cerr << el;
