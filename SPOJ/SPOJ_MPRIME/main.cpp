...
/*
    1. Problem Briefing: Given an integer K, find the K-th prime that created from
    concatenating two consecutive primes

    2. Keyword: brute-force

    3. Solution: Brute force
...
bool check_prime(ll x)
{
    if(x <= 1) return 0;
    if(x == 2) return 1;
    FOR(i, 2, ll(sqrt(x))+1) if(x % i == 0) return 0;
    return 1;
}
