...
void Solve()
{
    FOR(i, 1, n) if(avail[i]) DFS(i, 0);
    FOR(i, 1, k)
    {
        int ok = 1;
        rabb = wolf = vi(n+1, inf);
        BFS(queries[i].fi, wolf);
        BFS(queries[i].se, rabb);

        FOR(i, 1, n) if(isCyclic[i] && rabb[i] < wolf[i])
        {
            ok = 0;
            break;
        }
