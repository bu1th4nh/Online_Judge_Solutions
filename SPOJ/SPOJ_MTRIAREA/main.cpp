...
void Solve()
{
    ...
    for(; u < n; ++u)
    {
        ...
        while(1)
        {
            while(absArea(a[u], a[v], a[w]) <= absArea(a[u], a[v], a[(w+1)%n])) w = (w + 1) % n;
            if   (absArea(a[u], a[v], a[w]) <= absArea(a[u], a[(v+1)%n], a[w]))
            {
                v = (v + 1) % n;
                continue;
            }
        ...
        maximize(res, absArea(a[u], a[v], a[w]));
    }
